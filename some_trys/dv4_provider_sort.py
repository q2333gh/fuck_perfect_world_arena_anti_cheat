...
with open(os.path.join(script_dir, "DriversList.txt"), "w") as file:
    # Write the header
    file.write(
        f"{'DisplayName':<90} {'PathName':<120} {'State':<15} {'StartMode':<15} {'InstallDate':<25} {'ProviderName':<25}\n"
    )
    file.write("=" * 180 + "\n")

    # Query the Win32_SystemDriver class and sort by ProviderName
    drivers = sorted(
        c.Win32_SystemDriver(), key=lambda driver: driver.ProviderName or ""
    )
    ...
    for driver in drivers:
        ...
        provider_name = driver.ProviderName or "N/A"

        # Write the driver information to the file
        file.write(
            f"{display_name:<90} {path_name:<120} {state:<15} {start_mode:<15} {install_date:<25} {provider_name:<25}\n"
        )
